## List Users and Export Distribution Groups from M365 ##
            ##  Written by Mitchel Anderson ##
                      ## 2024 ##
...
"""
Due to Windows 11's configuration for PowerShell scripts, (specifically, Execution Policy),
you'll need to launch it through a PowerShell window.

To do so, run the script using the following command:

'.\view_and_export_distribution_groups.ps1'

You can also navigate to the directory where the script is, and begin typing the name, 
then hitting tab should autocomplete the entry for you.
"""


# INITIAL CONFIGURATION #
"""
The script will install the required modules to connect to the M365 Tenant, as well as export the data.
You'LL need to elevate the script the first time it's run so it can install these.

Subsequent runs will not need to be elevated.
"""


### CONNECTING TO M365 ###
"""
Due to the nature of M365, you must connect with a 365 Admin Account that can access both 365 and Exchange Online.
This script will connect, and disconnect each time it is run.
"""
